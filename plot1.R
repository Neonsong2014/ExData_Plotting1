# Name: plot1
# Purpose: plot a histogram and save it to a png file.
# Plot1 histogram of Global active power for the given days 
# and save the plot as plot1.png
...
plot_1<- function (dates) {
  
  path = getwd()
  png(filename = paste(path,"/plot1.png", sep=""), width=480, height= 480, units="px", pointsize = 12,     bg="white")
  sourcedf <- createdataframe(dates, 10000)
  hist(as.numeric(sourcedf$Global_active_power),  col="red", xlab="Global Active Power (kilowatts)", main="Global Active Power")
  dev.off()
}
